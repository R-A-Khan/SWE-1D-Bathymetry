...
clear all
xmax = 3;
x0_min_vec = [0.1*xmax 0.5*xmax (2/3)*xmax];

for l = 1:length(x0_min_vec)
fprintf('Commencing with x0_min = %0.2f', x0_min_vec(l));
% Fixed parameters for the data assimilation calculations
cfl         = 1/3;
n_obs       = [10];% Number of observation points
...
x0_min      =  x0_min_vec(l);     % Location of first observation point
case_num    = 1;
dmu         = 0.08;     % Spacing between observation points
...
%%
str = sprintf('Bath/Case DAB1D28/DAB1D28_case_%d',l);
% Saving Data
save(str, 'ntrial', 'n_obs', 'rand_x0','x0_min', 'dmu', 'line_min','smooth_grad', 'iter_max', ...
    'eta_exct0', 'beta_optimum', 'beta_exct0', 'X','x0_vals', 'err', 'err_std', 'grad', 'eta_error', 'eta_err_end', 'err_end','grad_std', 'cost', 'cost_std', 'Y_ex', 'Y_opt', 'Y_adj_opt', 'x0_inds');
