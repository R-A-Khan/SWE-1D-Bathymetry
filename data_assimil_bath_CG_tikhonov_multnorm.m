...
%                         options = optimset('Display', 'off', 'Tolfun',1e-4,'TolX', 1e4);
                    ...
                    if print_iter
                    disp([sprintf('%1.f',iter),'   ' sprintf('%0.4e',tau_n), '   ' sprintf('%0.3e',fval),'       ' sprintf('%1.0f',exitflag),'          ' sprintf('%0.3e',norm(cost(iter,jtrial))), '          ' sprintf('%0.3e',norm(grad_J)),'          ',sprintf('%0.3e',norm(err(iter,jtrial)))]);
                    end
