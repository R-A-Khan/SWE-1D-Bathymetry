...
function [eta_exct0, beta_optimum, beta_exct0, X, err, err_std, grad, grad_std, cost, cost_std,  Y_ex, Y_opt, Y_adj_opt, x0_inds] = ...
...
%     disp(['iter   ', 'tau_n            ' 'cost          ' , 'grad J             ','error']);
...
%             beta0(:,iter+1)= beta0(:,iter) + tau_n*(- grad_J_test);
            ...
            % optimised Bath
            H4 = [eta_exct0;u0];
            [eta_opt, u_opt, eta_x0_t_mult2, ~, T2, Y_opt] = FW_solve_bath(h, dt, H2, beta0(:,iter+1), 0, tmax, x0_inds, fw);
            H5 = zeros(2*N,1);
            [~, ~, ~, Y_adj_opt] = BW_solve_bath(h, dt, H5, 0, tmax, x0_inds, bw, obs_x0_mult, eta_x0_t_mult2, beta0(:,iter+1), u_opt, eta_opt, T1, T2);
            ...
            if print_iter
                if line_min
            disp([sprintf('%1.f',iter),'   ' sprintf('%0.4e',tau_n), '   ' sprintf('%0.7e',fval),'       ' sprintf('%1.0f',exitflag),'          ' sprintf('%0.9e',norm(cost(iter,jtrial))), '          ' sprintf('%0.9e',grad(iter,jtrial)),'          ',sprintf('%0.9e',norm(err(iter,jtrial)))]);
                else
