...
amp_bathy = [0.01:0.01:0.3];            % amplitude of bathymetry (0.3 of full depth is max for periodic ic)
...
amp_ic    = 	0.001; % amplitude of initial condition
    ...
    for k = 1: length(amp_bathy)
        dmu       = abs((xmax-1e-4) - x0_min)/n_obs;
         [eta_exct0(:,k), beta_optimum(:,k), beta_exct0(:,k), X, err(:,k), eta_error(:,k),err_end(k),eta_err_end(k), err_std(:,k), grad(:,k), grad_std(:,k), cost(:,k), cost_std, Y_ex(:,:,k), Y_opt(:,:,k), Y_adj_opt(:,:,k), x0_inds] = ...
           data_assimil_bath_2020_flip_2(N,n_obs,rand_x0,x0_min,dmu, ntrial,tmax,xmax,iter_max,line_min,smooth_grad,filt, tau_n, amp_bathy(k), ic_per, bathy_per, ...
    bathy_np_type, amp_ic, k_ic, k_bathy, phi_bathy, conj_grad_type, iter_chunk, a, c, cfl, noisy_obs, print_iter);
    end
    disp(' ');
    
%%
str = sprintf('Bath/Case DAB1D29/DAB1D29_case_%d',case_num);
% Saving Data
